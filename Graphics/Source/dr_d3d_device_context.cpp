  ...
  UInt32 stencilValue) const
{
  ...
  D3D11DeviceContext->ClearDepthStencilView(static_cast<D3DDepthStencil*>(&depthstencil)->APIDepthView.Get(), clearFlags, depthValue, stencilValue);
}

void D3DDeviceContext::clearRenderTargetView(RenderTarget& renderTarget, const float colorRGBA[4]) const
{
  for (size_t i = 0; i < static_cast<D3DRenderTarget*>(&renderTarget)->APIColorViews.size(); i++)
  {
    D3D11DeviceContext->ClearRenderTargetView(static_cast<D3DRenderTarget*>(&renderTarget)->APIColorViews[i].Get(), colorRGBA);
  }
}
...
void D3DDeviceContext::updateTextureFromMemory(Texture& texture, const char * databuffer, size_t bufferSize) const
{
  texture.udpateFromMemory(*this, databuffer, bufferSize);
}

void D3DDeviceContext::updateBufferFromMemory(Buffer & buffer, const char * dataBuffer, size_t bufferSize) const
{
  buffer.updateFromMemory(*this, dataBuffer, bufferSize);
}

void D3DDeviceContext::setRenderTarget(const RenderTarget & renderTarget, const DepthStencil & depthStencil) const
{
  renderTarget.set(*this, depthStencil);
}

void D3DDeviceContext::setTexture(const Texture & texture, UInt32 slot) const
{
...
void D3DDeviceContext::setConstantBuffer(const ConstantBuffer& buffer, DR_SHADER_TYPE_FLAG::E typeFlag) const
{
  buffer.set(*this, typeFlag);
}

void D3DDeviceContext::setPrimitiveTopology(DR_PRIMITIVE_TOPOLOGY::E topology) const
{

  D3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST); //Hardcoded
}
void D3DDeviceContext::draw(UInt32 indexCount, UInt32 startIndexLocation, UInt32 startVertexLocation) const
{
