...
namespace driderSDK {
  DR_GRAPHICS_ERROR::E D3DRenderTarget::create(const Device & device, const DrTextureDesc & colorDesc, UInt32 _numColorTextures)
  {
    ...
    for (size_t i = 0; i < numColorTextures; ++i)
    {
      ...
      if (static_cast<const D3DDevice*>(&device)->D3D11Device->CreateRenderTargetView(static_cast<D3DTexture*>(colorTextures.back())->APITexture.Get(), NULL, &RTV) != S_OK)
      {
  ...
  {
    static_cast<const D3DDeviceContext*>(&deviceContext)->D3D11DeviceContext->OMSetRenderTargets(numColorTextures,APIColorViews.back().GetAddressOf(), static_cast<const D3DDepthStencil*>(&depthStencil)->APIDepthView.Get());
  }
