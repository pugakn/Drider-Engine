...
PhysicsManager::TestCollision() {
  ...
  for (auto it: instance().m_lastCollidingItems) {
    if (instance().m_currrentCollidingItems.find(it.first) ==
        instance().m_currrentCollidingItems.end()) {
...
void
PhysicsManager::setGravity(float g) {
  instance().m_dynamicWorld->setGravity(reactphysics3d::Vector3(0.0f, g, 0.0f));
}

void 
PhysicsManager::onStartUp() {
  instance().m_world = new rp3d::CollisionWorld(); 
  m_gravity = rp3d::Vector3(0.0f, -9.81f, 0.0f);
