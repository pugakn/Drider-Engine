...
int main(int argc, char **argv)
{
  ...
  wcout << endl << "Press any key to quit." << endl;
  while (!_getch());
...
int RunApplication()
{
  ...
  if (engine == 0)
  {
    wcout << "Failed to create script engine." << endl;
    return -1;
  }
  ...
  if (ctx == 0)
  {
    wcout << "Failed to create the context." << endl;
    engine->Release();
    return -1;
  }
  ...
  if (r < 0)
  {
    wcout << "Failed to set the line callback function." << endl;
    ctx->Release();
    engine->Release();
    return -1;
  }
  ...
  if (func == 0)
  {
    wcout << "The function 'float calc(float, float)' was not found." << endl;
    ctx->Release();
    engine->Release();
    return -1;
  }
  ...
  if (r < 0)
  {
    wcout << "Failed to prepare the context." << endl;
    ctx->Release();
    engine->Release();
    return -1;
  }
  ...
  wcout << "Executing the script." << endl;
  //wcout << "---" << endl;
  r = ctx->Execute();
  //wcout << "---" << endl;
  if (r != asEXECUTION_FINISHED)
  {
    ...
    if (r == asEXECUTION_ABORTED)
      wcout << "The script was aborted before it could finish. Probably it timed out." << endl;
    else if (r == asEXECUTION_EXCEPTION)
    {
      wcout << "The script ended with an exception." << endl;

      // Write some information about the script exception
      asIScriptFunction *func = ctx->GetExceptionFunction();
      wcout << "func: " << func->GetDeclaration() << endl;
      wcout << "modl: " << func->GetModuleName() << endl;
      wcout << "sect: " << func->GetScriptSectionName() << endl;
      wcout << "line: " << ctx->GetExceptionLineNumber() << endl;
      wcout << "desc: " << ctx->GetExceptionString() << endl;
    }
    else
      wcout << "The script ended for some unforeseen reason (" << r << ")." << endl;
  }
  else
  {
    // Retrieve the return value from the context
    float returnValue = ctx->GetReturnFloat();
    wcout << "The script function returned: " << returnValue << endl;
  }
...
int CompileScript(asIScriptEngine *engine)
{
  ...
  if (f == 0)
  {
    wcout << "Failed to open the script file 'script.as'." << endl;
    return -1;
  }
  ...
  if (c == 0)
  {
    wcout << "Failed to load script file." << endl;
    return -1;
  }
...
void PrintString_Generic(asIScriptGeneric *gen)
{
  ...
  wcout << StringUtils::toTString(*str);
}
